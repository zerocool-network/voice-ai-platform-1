...
function AnalyzeNode({ data, selected }) {
  const invalid = data._valid === false;

  return (
    <div className={`relative min-w-48 rounded-xl border shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 dark:bg-zinc-900 ${selected ? 'ring-2 ring-cyan-500/25 border-cyan-400 dark:ring-cyan-400/30' : ''} ${invalid ? '!border-red-500' : 'border-indigo-200 dark:border-indigo-800'}`}>
      ...
      <Handle type="target" position={Position.Top} className="!size-3 !border-2 !border-indigo-400 !bg-white" />
      <Handle type="source" position={Position.Bottom} className="!size-3 !border-2 !border-indigo-400 !bg-white" />
    </div>
  );
}
